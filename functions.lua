...
-- ============================================================
--  PELLET REDIRECT  (intercept + modify natural shotgun fires)
-- ============================================================
--  Hooks MainEvent:FireServer("Shoot", payload). When active AND a
--  valid ragebot target exists, rewrites each pellet's Instance /
--  Position / theOffset so the pellet "lands" on a body part of the
--  target -- BUT only if the redirected pellet's direction stays
--  within MAX_ANGLE_DEG of the natural direction. Pellets whose
--  natural hit is too far from the target stay unmodified.
--
--  This piggy-backs on the gun's natural validation: we don't
--  fabricate origin, timestamp, normals, cone shape, or any other
--  field that might be checked. We only swap which Instance got
--  hit. Server can still validate spread cone (still natural),
--  origin (still the gun barrel), payload shape (untouched).
--
--  Gated by getgenv()._F_PR_ACTIVE so cross-script reloads stay
--  safe. checkcaller() filters out our own forceHit calls.
-- ============================================================
getgenv()._F_PR_ACTIVE = getgenv()._F_PR_ACTIVE or false
getgenv()._F_PR_MAX_ANGLE = getgenv()._F_PR_MAX_ANGLE or 15  -- degrees
getgenv()._F_PR_DEBUG = getgenv()._F_PR_DEBUG or false
if hookmetamethod and not getgenv()._F_PR_NAMECALL_HOOKED then
    getgenv()._F_PR_NAMECALL_HOOKED = true

    local function _prGetTarget()
        if rbGetTarget then
            local p = rbGetTarget()
            if p and p.Parent and p.Character then return p end
        end
        return nil
    end

    local function _prBodyParts(char)
        local out = {}
    ...
    prOldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
        ...
        local args = {...}
        local self = args[1]
        if typeof(self) ~= "Instance" then return prOldNamecall(...) end
        if self.Name ~= "MainEvent" then return prOldNamecall(...) end
...
-- Pellet redirect: intercepts natural Shoot fires and redirects each
-- pellet to a body part of the current ragebot target, while keeping
-- the pellet's direction within MaxAngle of its natural direction.
-- Lets the gun produce a real spread pattern -> we just swap hit targets.
F.pelletRedirect = {
    start  = function() getgenv()._F_PR_ACTIVE = true end,
    stop   = function() getgenv()._F_PR_ACTIVE = false end,
    toggle = function()
        getgenv()._F_PR_ACTIVE = not getgenv()._F_PR_ACTIVE
    end,
    isActive    = function() return getgenv()._F_PR_ACTIVE == true end,
    setMaxAngle = function(n) getgenv()._F_PR_MAX_ANGLE = tonumber(n) or 15 end,
    getMaxAngle = function() return tonumber(getgenv()._F_PR_MAX_ANGLE) or 15 end,
    setDebug    = function(b) getgenv()._F_PR_DEBUG = b == true end,
    getDebug    = function() return getgenv()._F_PR_DEBUG == true end,
}
...
F.desync = (function()
    ...
    -- ============================================================
    --  Ghost marker (raknet desync visual)
    --  When raknet desync turns on, we capture HRP.Position and build
    --  a frozen clone of the character at that spot. It shows the
    --  position the server thinks we're at while we run free. Comes
    --  with rotating ground rings + lightning sparks so it's visible
    --  through walls.
    -- ============================================================
    local PI2          = math.pi * 2
    ...
    local ghostModel, ghostVfxConn
    local ghostGroundParts = {}
    local ghostAttachments = {}

    local function ghostRemove()
        if ghostVfxConn then ghostVfxConn:Disconnect(); ghostVfxConn = nil end
        for _, p in ipairs(ghostGroundParts) do
    ...
    local function ghostMakeSparks(parent)
        local att = Instance.new("Attachment"); att.Parent = parent
        local em = Instance.new("ParticleEmitter")
        em.Color = lightningColor; em.LightEmission = 1; em.LightInfluence = 0
        em.Size = sparkSize
        em.Lifetime = NumberRange.new(0.1, 0.3)
        em.Rate = 0; em.Speed = NumberRange.new(5, 20)
    ...
    local function ghostMakeRingDot(px, py, pz, sz, col, sparksSize)
        local dot = Instance.new("Part")
        dot.Anchored = true; dot.CanCollide = false
        dot.CanTouch = false; dot.CanQuery = false; dot.CastShadow = false
        dot.Size = Vector3.new(sz, sz, sz)
        dot.Shape = Enum.PartType.Ball
        dot.Material = Enum.Material.Neon
        dot.Color = col; dot.Transparency = 0
        dot.CFrame = CFrame.new(px, py, pz); dot.Parent = workspace
        local att = Instance.new("Attachment"); att.Parent = dot
        local em = Instance.new("ParticleEmitter")
        em.Color = lightningColor; em.LightEmission = 1; em.LightInfluence = 0
        em.Size = sparksSize
        em.Lifetime = NumberRange.new(0.1, 0.2)
        em.Rate = 10; em.Speed = NumberRange.new(1, 5)
        em.SpreadAngle = Vector2.new(180, 180)
        em.Parent = att
        return dot
    end
    ...
    local function ghostCreate(pos)
        ...
        for _, v in ipairs(ghost:GetDescendants()) do
            pcall(function()
                ...
                or v:IsA("Animator") or v:IsA("AnimationController") then v:Destroy() end
            end)
        end
        ...
        for _, v in ipairs(ghost:GetDescendants()) do
            pcall(function()
                if v:IsA("BasePart") then
                    ...
                    v.CanTouch = false; v.CanQuery = false; v.CastShadow = false
                    v.Transparency = 0; v.Material = Enum.Material.SmoothPlastic
                    v.Color = Color3.fromRGB(0, 20, 80)
                    v.CFrame = v.CFrame + offset
                    if v.Name ~= "HumanoidRootPart" then
                        for _ = 1, 3 do
                            table.insert(ghostAttachments, ghostMakeSparks(v))
                        end
                    end
                end
            end)
        ...
        hl.FillColor = Color3.fromRGB(0, 60, 180); hl.FillTransparency = 0
        hl.OutlineColor = Color3.fromRGB(0, 120, 255); hl.OutlineTransparency = 0
        hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
        hl.Adornee = ghost; hl.Parent = ghost

        ghost.Parent = workspace
        ghostModel = ghost

        -- ground rings (32 outer, 20 inner)
        ...
        for i = 1, rCount do
            ...
            table.insert(ghostGroundParts, {
                dot = ghostMakeRingDot(pos.X + math.cos(a) * OUTER_RADIUS, gy,
                                       pos.Z + math.sin(a) * OUTER_RADIUS,
                                       0.25, outerCol, ringSizeOuter),
                baseAngle = a, isOuter = true,
            })
        end
        for i = 1, iCount do
            local a = (i / iCount) * PI2
            table.insert(ghostGroundParts, {
                dot = ghostMakeRingDot(pos.X + math.cos(a) * INNER_RADIUS, gy,
                                       pos.Z + math.sin(a) * INNER_RADIUS,
                                       0.15, innerCol, ringSizeInner),
                baseAngle = a, isOuter = false,
            })
        end

        local sparkTimer = 0
        ghostVfxConn = RunService.Heartbeat:Connect(function(dt)
            ...
            for _, entry in ipairs(ghostGroundParts) do
                if entry.dot and entry.dot.Parent then
                    ...
                    entry.dot.CFrame = CFrame.new(pos.X + math.cos(a) * radius,
                                                  gy + wave,
            ...
            if sparkTimer >= SPARK_INTERVAL then
                sparkTimer = 0
                local n = #ghostAttachments
                if n > 0 then
                    for _ = 1, math.random(2, 5) do
                        local pick = ghostAttachments[math.random(1, n)]
                        if pick and pick.Parent then
                            local em = pick:FindFirstChildOfClass("ParticleEmitter")
                            if em then em:Emit(math.random(8, 25)) end
                        end
                    end
                end
            end
