...
-- ============================================================
--  ACCESS WHITELIST CHECK  (client-side, best-effort)
-- ============================================================
--  Fetches wl.cclosure from the repo and verifies the local
--  player's UserId is in the allowed list. Kicks otherwise.
--
--  IMPORTANT — this check runs on the user's machine and is
--  fundamentally bypassable by anyone who edits the loader before
--  running it. The only truly spoof-proof setup is server-side
--  validation (your own HTTP endpoint that gates the response).
--  This check is here to discourage casual sharing — anyone who
--  bothers to read the source can no-op it.
--
--  Anti-bypass measures used:
--    * UserId match (immutable) rather than Name (changeable)
--    * Cache-buster on URL so we always fetch fresh
--    * Pcall'd JSON parse + structure validation
--    * Network/parse failure -> kick (don't fail open)
--    * Periodic re-check every 60s while the script is running
-- ============================================================
do
    local Players      = game:GetService("Players")
    local HttpService  = game:GetService("HttpService")
    local lplr         = Players.LocalPlayer
    local WL_URL       = "https://raw.githubusercontent.com/anxiousgh/hfgfghoifghfgkm-h/main/wl.cclosure"

    local function fetchWl()
        local ok, body = pcall(function()
            return game:HttpGet(WL_URL .. "?_=" .. tick())
        end)
        ...
        if not ok2 or type(decoded) ~= "table" or type(decoded.users) ~= "table" then
            return nil
        end
        return decoded
    end

    local function isAllowed(wl)
        if not wl or not wl.users then return false end
        for _, entry in ipairs(wl.users) do
            if type(entry) == "table" and entry.userId == lplr.UserId then
                return true
            end
        end
        return false
    ...
    local wl = fetchWl()
    if not wl then
        lplr:Kick("[cclosure.vip] Could not verify access. Try again.")
        return
    end
    if not isAllowed(wl) then
        lplr:Kick(("[cclosure.vip] Not whitelisted.\nUserId: %d"):format(lplr.UserId))
        return
    end

    -- Periodic re-check: every 60s, refetch the WL and kick if the
    -- user was removed. Survives toggle on/off, runs forever.
    task.spawn(function()
        while true do
            task.wait(60)
            local fresh = fetchWl()
            if fresh and not isAllowed(fresh) then
                lplr:Kick("[cclosure.vip] Access revoked.")
                return
            end
        end
    end)
